...
class MTNER:
    ...
    def _prediction_loop(
        ...
        go_preds: torch.Tensor = None
        bp_preds: torch.Tensor = None
        diag_preds: torch.Tensor = None
        treat_preds: torch.Tensor = None
        #mut_preds: torch.Tensor = None
        loc_preds: torch.Tensor = None
        food_preds: torch.Tensor = None
        ...
        for inputs in tqdm(dataloader, desc=description):
            ...
            if not prediction_loss_only:
                (dise_logits, chem_logits, ct_logits, go_logits, bp_logits, diag_logits, treat_logits,loc_logits,food_logits) = logits
                if dise_preds is None and chem_preds is None and ct_preds is None and go_preds is None and bp_preds is None and diag_preds is None and treat_preds is None and loc_preds is None and food_preds is None:
                    ...
                    go_preds = go_logits.detach()
                    bp_preds = bp_logits.detach()
                    diag_preds = diag_logits.detach()
                    treat_preds = treat_logits.detach()
                    #mut_preds = mut_logits.detach()
                    loc_preds = loc_logits.detach()
                    food_preds = food_logits.detach()
                else:
                    ...
                    go_preds = torch.cat((go_preds, go_logits.detach()), dim=0)
                    bp_preds = torch.cat((bp_preds, bp_logits.detach()), dim=0)
                    diag_preds = torch.cat((diag_preds, diag_logits.detach()), dim=0)
                    treat_preds = torch.cat((treat_preds, treat_logits.detach()), dim=0)
                    #mut_preds = torch.cat((mut_preds, mut_logits.detach()), dim=0)
                    loc_preds = torch.cat((loc_preds, loc_logits.detach()), dim=0)
                    food_preds = torch.cat((food_preds, food_logits.detach()), dim=0)
        ...
        if dise_preds is not None and chem_preds is not None :
            ...
            go_preds = go_preds.cpu().numpy()
            bp_preds = bp_preds.cpu().numpy()
            diag_preds = diag_preds.cpu().numpy()
            treat_preds = treat_preds.cpu().numpy()
            #mut_preds = mut_preds.cpu().numpy()
            loc_preds = loc_preds.cpu().numpy()
            food_preds = food_preds.cpu().numpy()
        ...
        return_output = (PredictionOutput(predictions=dise_preds, label_ids=label_ids), \
                        ...
                        PredictionOutput(predictions=go_preds, label_ids=label_ids), \
                        PredictionOutput(predictions=bp_preds, label_ids=label_ids), \
                        PredictionOutput(predictions=diag_preds, label_ids=label_ids), \
                        PredictionOutput(predictions=treat_preds, label_ids=label_ids), \
                        #PredictionOutput(predictions=mut_preds, label_ids=label_ids), \
                        PredictionOutput(predictions=loc_preds, label_ids=label_ids), \
                        PredictionOutput(predictions=food_preds, label_ids=label_ids))
