...
def pubtator2dict_list(pubtator_file_path, is_raw_text=False):
    dict_list = list()
    title_pmid = ''
    # abstract_pmid = ''
    title = ''
    ...
    with open(pubtator_file_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if len(line) == 0:
                ...
                if len(mentions) > 0:
                    ...
                    if is_raw_text:
                        ...
                        mentions = get_bestplus_spans(mentions,
                                                       title + ' ' +
                                                       abstract_text)
                ...
                doc_dict = {
                    'pmid': title_pmid,
                    'entities': {'mention': copy.deepcopy(mentions)}
                }
                if is_raw_text:
            ...
            if doc_line_num == 0:
                ...
                if len(mention_cols) != 6:
                    mentions.append({'start': int(mention_cols[1]),
                                  ...
                                  'mention': mention_cols[3],
                                  'subtype': mention_cols[4],
                                  'id': ''})
                else:
                    mentions.append({'start': int(mention_cols[1]),
                                  'end': int(mention_cols[2]),
            ...
            doc_line_num += 1
    return dict_list
def reformat_tmvar(tmvar_docs):
    for tmvar_doc in tmvar_docs: 
        tmvar_doc['entities']['mutation']=[]
        for entity in tmvar_doc['entities']['mention']: 
            nor_items=entity['id'].split(';')
            nor_mut=''
            cpg=''
            cps=''
            for item in nor_items:
                ...
                if item[0]=='tmVar':
                    nor_mut=item[1]
                elif item[0]=='CorrespondingGene':
                    cpg=item[1]
                    entity['CorrespondingGene']=cpg
                elif item[0]=='CorrespondingSpecies':
                    cps=item[1]
                    entity['CorrespondingSpecies']=cps
            entity['id']=nor_mut
            tmvar_doc['entities']['mutation'].append(entity)
        del tmvar_doc['entities']['mention']
    return tmvar_docs

def reformat_gnorm(gnorm_docs):
    cellline_docs=[]
    for gnorm_doc in gnorm_docs: 
        cellline_doc=copy.deepcopy(gnorm_doc)
        cellline_doc['entities']['cellline']=[]
        gnorm_doc['entities']['gene']=[]
        gnorm_doc['entities']['species']=[]
        del cellline_doc['entities']['mention']
        for entity in gnorm_doc['entities']['mention']: 
            entity_type=entity['subtype'].lower()
            if entity_type in ['cellline','gene']:
                del entity['subtype']
                if entity_type=='cellline':
                    del entity['id']
                    cellline_doc['entities']['cellline'].append(entity)
                else:
    ...
    return gnorm_docs,cellline_docs
...
def get_bestplus_spans(mentions, title_space_abstract):
    adjusted_mentions = list()
    ...
    for m in mentions:
        ...
        if m['mention'] in mention_count_dict:
            ...
            start = -1
            found = 0
            pattern = r'\b' + re.escape(m['mention']) + r'\b'
            '''
            while found < count:
                ...
                found += 1
            '''
            for match in re.finditer(pattern, title_space_abstract):
                start = match.start()
                end=match.end()
                found += 1
                ...
                assert m['mention'] == title_space_abstract[start: end]
    
                adjusted_mentions.append({'start': start,
                                           'end': end,
                                           'mention': m['mention'],
                                           'subtype': m['subtype'],
                                           'id': m['id']})
