...
def read_examples_from_file(data_dir, mode: Union[Split, str], data_list='', eval_data_list='') -> List[InputExample]:
    ...
    def get_merge_idx(data_name):
        if data_name in ["NCBI-disease", "BC5CDR-disease", "mirna-di", "ncbi_disease", "scai_disease", "variome-di","disease"]:
            merge_idx = 1
        elif data_name in ["BC5CDR-chem",  "cdr-ch", "chemdner", "scai_chemicals", "chebi-ch", "BC4CHEMD","chemical"]:
            merge_idx = 2
        elif data_name in ["BC2GM", "JNLPBA-protein", "bc2gm", "mirna-gp", "cell_finder-gp", "chebi-gp", "loctext-gp", "deca", "fsu", "gpro", "jnlpba-gp", "bio_infer-gp", "variome-gp", "osiris-gp",  "iepa","gene"]:
            merge_idx = 3
        elif data_name in ["s800", "linnaeus", "loctext-sp", "mirna-sp", "chebi-sp", "cell_finder-sp", "variome-sp","species"]:
            merge_idx = 4
        elif data_name in ["Revised_JNLPBA-ct","JNLPBA-ct","jnlpba-ct","Medmention-ct","NERO-ct","cell_type","JNLPBA-cl", "cell_finder-cl", "jnlpba-cl", "gellus", "cll","cell_line","celltype"]:
            merge_idx = 5
        elif data_name in ["Medmention-go","CRAFT-go","gene_ontology","go"]:
            merge_idx = 6
        elif data_name in ["Medmention-bp","NERO-bp","body_part","bodypart"]:
            merge_idx = 7
        elif data_name in ["Medmention-diagnose","diagnose"]:
            merge_idx =8
        elif data_name in ["Medmention-treatment","treatment"]:
            merge_idx = 9
        elif data_name in ["VIMRT-mutation","mutation"]:
            merge_idx = 10
        elif data_name in ["CONLL2003","Medmention_location","location"]:
            merge_idx = 11
        elif data_name in ["CAFETERIA","food"]:
            merge_idx = 12
        else:
            merge_idx = 0
        return merge_idx
