...
class NeuralNormalizer(object):
    ...
    def normalize(self, names, ent_type):
        ...
        start_time = time.time()
        distances, indices = self.dict_embeds[ent_type].search(name_embeds, self.k)
        start_time = time.time()
        top1s = indices[:,0]

        assert (len(top1s) == len(names))

        outputs = [self.dictionary[ent_type][top1] if top1 != -1 else (-1, -1) for top1 in top1s]
        return outputs
    ...
    def load_cache(self, cache_path):
        self.dict_embeds={}
        self.dictionary={}
        for ent_type in cache_path:
            self.dict_embeds[ent_type] = faiss.read_index(cache_path[ent_type] + ".index")
            with open(cache_path[ent_type], 'rb') as f:
                self.dictionary[ent_type] = pickle.load(f)
